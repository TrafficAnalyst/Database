...
api_url = "http://127.0.0.1:5000/savedata"
...
analisis_data = {
    ...
    response = requests.post(f"http://127.0.0.1:5000/insert_kendaraan", json=kendaraan_data)
    ...
    response = requests.post(f"http://127.0.0.1:5000/insert_video", json=video_data)
    if response.status_code == 200:
    ...
    response = requests.post(f"http://127.0.0.1:5000/insert_analisis", json=analisis_data)
    if response.status_code == 200:
